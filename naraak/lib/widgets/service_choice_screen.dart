...
class ServiceChoiceScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: NaraakAppBar(title: strings.raw(title)),
      body: ResponsivePageFrame(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(strings.raw(prompt), style: AppTextStyles.h2),
          const SizedBox(height: 14),
          LayoutBuilder(builder: (context, constraints) {
            final cards = choices
                .map((choice) => _ServiceChoiceCard(choice: choice))
                .toList();
      ...
      child: Row(children: [
        Container(
            ...
            decoration: BoxDecoration(
                color: palette.primary.withValues(alpha: isDark ? .2 : .1),
                borderRadius: BorderRadius.circular(14)),
            child: Icon(choice.icon, color: palette.primary, size: 25)),
        const SizedBox(width: 14),
        Expanded(
            child:
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(strings.raw(choice.title), style: AppTextStyles.h3),
          const SizedBox(height: 4),
          Text(strings.raw(choice.description),
